...
void no_person_detection()
{
    // get person detection image
    memcpy(input->data.int8, g_no_person_image_data, kNumCols * kNumRows * kNumChannels);
    //memcpy(input->data.int8, g_no_person_image_data, kNumCols * kNumRows * kNumChannels);
    // Run the model on this input and make sure it succeeds.
    if (kTfLiteOk != interpreter->Invoke())
    {
...
{
    // get person detection image
    memcpy(input->data.int8, g_person_image_data, kNumCols * kNumRows * kNumChannels);
    //memcpy(input->data.int8, g_person_image_data, kNumCols * kNumRows * kNumChannels);
    // Run the model on this input and make sure it succeeds.
    if (kTfLiteOk != interpreter->Invoke())
    {
