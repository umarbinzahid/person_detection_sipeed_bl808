...
struct EnumDef : public Definition {
    ...
    {
        return Distance(MinValue(), MaxValue());
    }
...
{
    return IsStruct(type) ? type.struct_def->bytesize : (IsArray(type) ? InlineSize(type.VectorType()) * type.fixed_length : SizeOf(type.base_type));
}
